...
Deployment does not flip everything to automatic on day one. Setup runs 3 to 12 months, covering automations, integrations, tests, and go-live adjustments, with clear scope and KPIs agreed before the start, followed by continuous operation in production after go-live. WIR was built with Mahway, a Venture Builder in California, and Avante, a Venture Studio in Brazil. The current public traction is a POC in execution with a global insurer in the Transport line. To size the opportunity in a given portfolio, it is worth pairing this read with the [underwriting intelligence](#blog/inteligencia-de-subscricao-brasil-en) page and mapping which risks could already flow through STP. The AI layer for insurance. On top of the systems the insurer already runs, never in their place.`,
  ...
  },
  {
    ...
    author: "WIR Innovation", role: "Guia de automação",
    time: "7 min", date: "15 · Jun · 2026", dateISO: "2026-06-15",
    metaDesc: "Guia para seguradoras validarem e enriquecerem submissões com CNPJ, exposição e crédito por uma camada de IA externa, sobre os sistemas atuais. Veja como.",
...
A WIR nasceu de experiência operacional acumulada, construída com a Mahway, Venture Builder na Califórnia, e a Avante, Venture Studio no Brasil. A tração pública é direta e conservadora: uma primeira POC em execução com uma seguradora global no ramo de Transporte. Esse é o estágio real, e a melhor forma de avaliar a esteira sem toque é mapear, junto com a WIR, quais riscos da sua operação podem fluir sem toque e onde o subscritor deve entrar.`,
    faq: [
      { q: "Quais riscos podem passar pela esteira sem toque e quais escalam para humano?", a: "Fluem sem toque os riscos que atendem a todas as condições ao mesmo tempo: estar dentro do apetite, dentro da faixa de exposição e de alçada, completos e lidos com alta confiança de extração, e sem sinal de fraude. Falhou em qualquer uma, o caso escala para o subscritor. Quem define cada limite é a seguradora, no seu manual de subscrição. Assim o automático cuida do risco simples e o humano fica com o que exige julgamento." },
      { q: "A esteira sem toque substitui o core da seguradora?", a: "Não. A esteira opera como camada de IA externa, sobre os sistemas que a seguradora já usa. Não há migração de core e não há carga para o time de TI. A camada se integra ao core de apólice existente pelo formato que a operação já usa, lê e decide segundo a política de risco do cliente, e grava o resultado de volta no core com a trilha de auditoria. O sistema central da seguradora permanece no lugar." },
      { q: "Como a esteira sem toque respeita o manual de subscrição e o apetite?", a: "O Machine Learning é calibrado ao apetite de risco e ao manual de subscrição da seguradora durante a implantação, com base na política de aceitação e no histórico de sinistros. Por isso a decisão automática reflete a regra do próprio cliente, e não um padrão genérico. A seguradora define a faixa de elegibilidade que separa o que flui sozinho do que escala, e ajusta esses limites ao longo da operação conforme a carteira evolui. O controle do automático está na calibração." },
      { q: "Cada decisão automática gera uma trilha de auditoria?", a: "Sim. Cada decisão da esteira é explicável e retorna uma trilha de auditoria completa, com o registro dos fatores que levaram ao score, à recusa ou ao encaminhamento. A seguradora consegue reconstruir qualquer decisão depois e responder a uma auditoria. Os dados são criptografados em cada etapa e a operação é compatível com a LGPD. Essa explicabilidade sustenta o direito de revisão de decisões automatizadas, ainda mais importante num fluxo que decide de forma majoritariamente automática." },
      { q: "Quanto a esteira sem toque ajuda a escalar prêmio sem aumentar a equipe?", a: "Ao deixar o risco simples fluir do intake à decisão sem passo manual, a esteira libera o subscritor para tratar só a exceção, o que permite processar mais submissões com a mesma equipe. O ganho exato depende de quanto da carteira cabe na faixa sem toque, definida pela seguradora. A WIR não promete um número fixo: o caminho é mapear quais riscos podem fluir sem toque na sua operação e ampliar a faixa de forma gradual, monitorando a taxa de processamento automático." }
    ],
  },
  {
...
Yes. The output of triage is a prioritized, classified work queue with a visible SLA, not an undifferentiated inbox. Each submission is captured and timestamped on arrival, which starts the SLA clock, and each case is placed in the right lane with its classification and reasoning attached. Real-time dashboards give the team the right cases at the top, so the underwriter sees a desk ordered by priority and the broker gets a predictable response.`,
    faq: [
      ...
      { q: "What is the difference between automatic triage and underwriting routing?", a: "Triage is the intake step that sorts, classifies, and prioritizes the inbox. It answers what a submission is, whether it is complete, whether it fits appetite, and how urgent it is. Underwriting routing is the later decision step that produces a quote, a decline, or an escalation and sends the case to a specific underwriter. Triage feeds routing a queue that is already ordered by what matters, so routing acts on a sorted queue instead of a raw one." },
      { q: "Does triage work with submissions that arrive by e-mail and attachments?", a: "Yes. In P&C, submissions rarely arrive as clean structured data through one door. They come as e-mail bodies, PDF proposal forms, scanned documents, spreadsheets, and broker cover notes across several channels. Multichannel intake captures them on arrival, and intelligent document reading extracts the fields into structured data. Triage then consumes that structured data to classify each submission by line, product, and broker and to prioritize it by appetite and exposure." },
      { q: "Does the underwriting team see the queue and the priority order?", a: "Yes. The output of triage is a prioritized, classified work queue with a visible SLA, not an undifferentiated inbox. Each submission is captured and timestamped on arrival, which starts the SLA clock, and each case is placed in the right lane with its classification and reasoning attached. Real-time dashboards give the team the right cases at the top, so the underwriter sees a desk ordered by priority and the broker gets a predictable response." }
    ],
  },
  {
    slug: "manifesto",
...
The conversion score ranks the quotation queue so the highest-probability, in-appetite submissions reach the underwriter and broker first. This matters because Capgemini finds 60%+ of brokers choose an insurer by response speed. WIR orders the queue by appetite and exposure, turning an invisible pipeline into a ranked one, so winnable business stops leaking to whichever competitor answers faster.
...
Re-scoring and re-pricing in time keeps the account, which matters because Capgemini finds 60%+ of brokers choose an insurer by response speed. WIR's Smart Sales maps the portfolio by client and product and surfaces next-best-action upsell at the renewal moment, so penetration and retention grow together. Cross-sell and coverage-upgrade recommendations arrive exactly when the renewal decision is being made.
...
Brazil runs one of the largest insurance markets in Latin America and one of the larger emerging markets globally. The sector is supervised by SUSEP, the federal authority for private insurance, and represented by CNseg, with FenSeg covering the property and casualty federation. Sector revenue has been growing at double-digit rates, according to CNseg reporting for recent years.
...
Distribution adds the final pressure. Insurance in Brazil is heavily intermediated through brokers (corretores), and 60%+ of brokers choose an insurer by response speed, according to Capgemini. Brokers route business to whichever insurer quotes fast and consistently, so time-to-quote is a direct conversion lever. Slow or inconsistent quoting loses business at the point of sale. Digital channels, embedded insurance, and a growing field of InsurTechs keep compressing the response times brokers and clients expect.
