...
const T = {
  pt: {
    ...
    shiftHeadP: "Produto",
    shiftRows: [
      { dor:"E-mails de cotações desorganizados",   sol:"Estruturação e leitura automática de submissões", prod:"Smart Sales",            status:"prod" },
      { dor:"Falta de priorização comercial",        sol:"Classificação por apetite e rentabilidade",        prod:"Smart Sales",            status:"prod" },
      { dor:"Underwriter fazendo operação manual",   sol:"Eliminação completa das tarefas manuais",          prod:"Underwriter Intel",      status:"prod" },
      { dor:"Pipeline invisível para gestão",        sol:"Dashboard estratégico em tempo real",              prod:"Underwriter Intel",      status:"prod" },
      { dor:"Riscos fora do apetite processados",    sol:"Recusa automática inteligente",                    prod:"Smart Sales · UI",       status:"prod" },
    ...
    prods: [
      ...
      { k:"XBA", c:"#FE8B77", status:"dev", title:"X-sell Brokers", tag:"Automação de cross-sell",
      ...
      { k:"SNB", c:"#F8AD39", status:"dev", title:"SDR New Business", tag:"Venda inteligente",
  ...
  en: {
    ...
    shiftHeadP: "Product",
    shiftRows: [
      { dor:"Disorganized quote request emails",     sol:"Automatic structuring and reading of submissions", prod:"Smart Sales",          status:"prod" },
      { dor:"No commercial prioritization",          sol:"Classification by appetite and profitability",     prod:"Smart Sales",          status:"prod" },
      { dor:"Underwriters doing manual operations",  sol:"Complete elimination of manual tasks",             prod:"Underwriter Intel",    status:"prod" },
      { dor:"Pipeline invisible to management",      sol:"Strategic real-time dashboard",                    prod:"Underwriter Intel",    status:"prod" },
      { dor:"Out-of-appetite risks being processed", sol:"Intelligent automatic declination",                prod:"Smart Sales · UI",     status:"prod" },
    ...
    stackCta: "Explore",
    prods: [
      { k:"SS", c:"#3222E9", status:"prod", title:"Smart Sales", tag:"Distribution intelligence",
      ...
      { k:"UI", c:"#AE46C0", status:"prod", title:"Underwriter Intelligence", tag:"Intelligent underwriting",
      ...
      { k:"XBA", c:"#FE8B77", status:"dev", title:"X-sell Brokers", tag:"Cross-sell automation",
      ...
      { k:"SNB", c:"#F8AD39", status:"dev", title:"SDR New Business", tag:"Intelligent sales",
  ...
  es: {
    ...
    shiftHeadP: "Producto",
    shiftRows: [
      { dor:"Correos de cotización desorganizados",    sol:"Estructuración y lectura automática de solicitudes", prod:"Smart Sales",          status:"prod" },
      { dor:"Falta de priorización comercial",          sol:"Clasificación por apetito y rentabilidad",            prod:"Smart Sales",          status:"prod" },
      { dor:"Suscriptores haciendo operación manual",   sol:"Eliminación completa de las tareas manuales",         prod:"Underwriter Intel",    status:"prod" },
      { dor:"Pipeline invisible para la gerencia",      sol:"Dashboard estratégico en tiempo real",                prod:"Underwriter Intel",    status:"prod" },
      { dor:"Riesgos fuera del apetito procesados",     sol:"Declinación automática inteligente",                  prod:"Smart Sales · UI",     status:"prod" },
    ...
    prods: [
      ...
      { k:"XBA", c:"#FE8B77", status:"dev", title:"X-sell Brokers", tag:"Automatización de cross-sell",
      ...
      { k:"SNB", c:"#F8AD39", status:"dev", title:"SDR New Business", tag:"Venta inteligente",
...
export function ProductTabs({ go }) {
  ...
  const onTabKey = (e, i) => {
    if (e.key === "ArrowRight") { e.preventDefault(); const n = (i + 1) % prods.length; setActive(n); tabRefs.current[n]?.focus(); }
    else if (e.key === "ArrowLeft")  { e.preventDefault(); const n = (i - 1 + prods.length) % prods.length; setActive(n); tabRefs.current[n]?.focus(); }
    else if (e.key === "Home")       { e.preventDefault(); setActive(0); tabRefs.current[0]?.focus(); }
    else if (e.key === "End")        { e.preventDefault(); const n = prods.length - 1; setActive(n); tabRefs.current[n]?.focus(); }
  };
      ...
      <div className="wrap">
        ...
        <div className="stack__layout">
          ...
          <div className="stack__panel" style={{"--pc": p.c}} key={p.k}
            role="tabpanel" id={`stack-panel-${p.k}`} aria-labelledby={`stack-tab-${p.k}`} tabIndex={0}>
            ...
            <div className="stack__panel-bot">
              ...
              <button className="btn btn--ghost stack__cta" onClick={()=>go("contact")}>
                {T.stackCta} {p.k}
              </button>
            </div>
          </div>
